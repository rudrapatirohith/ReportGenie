  ...
  try {
    const resp = await fetch('/api/generate', {
      ...
      body: JSON.stringify({
        ...
        upcoming_tasks: [
          { task: $('#s-ut1').value.trim() || '-', date: toUSDate($('#s-ud1').value.trim()) || '' },
          { task: $('#s-ut2').value.trim() || '-', date: toUSDate($('#s-ud2').value.trim()) || '' },
          { task: $('#s-ut3').value.trim() || '-', date: toUSDate($('#s-ud3').value.trim()) || '' },
        ],
        mode: 'smart',
  ...
  try {
    const resp = await fetch('/api/generate', {
      ...
      body: JSON.stringify({
        ...
        upcoming_tasks: [
          { task: $('#a-ut1').value.trim() || '-', date: toUSDate($('#a-ud1').value.trim()) || '' },
          { task: $('#a-ut2').value.trim() || '-', date: toUSDate($('#a-ud2').value.trim()) || '' },
          { task: $('#a-ut3').value.trim() || '-', date: toUSDate($('#a-ud3').value.trim()) || '' },
        ],
        mode: 'ai',
